/**
 * Centralized Neo4j Visualization Module
 * Based on the latest version from neo4j-visualization-test
 * This module provides a unified interface for all Neo4j visualizations in the application
 */

import * as d3 from 'd3';

// Import the D3DiagramProduction component if in React environment
let D3DiagramProduction = null;
if (typeof React !== 'undefined') {
  D3DiagramProduction = await import('./components/D3DiagramProduction.js').then(m => m.default);
}

/**
 * Neo4jVisualization - Main class for non-React usage
        ...
        ">
          <div class="neo4j-visualization" style="flex: 1;">
            <svg class="neo4j-svg" width="${this.options.showHistory ? this.options.width * 0.7 : this.options.width}" 
                 height="${this.options.height - (this.options.showSearch || this.options.showFilters ? 80 : 0)}"></svg>
          </div>
...
// Export React component if available
export const Neo4jVisualizationControl = D3DiagramProduction;

// Global initialization function for non-module usage
